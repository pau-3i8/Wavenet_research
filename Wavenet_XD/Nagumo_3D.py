...
def euler(param, Iapp):
    ...
    for I in Is:
        ...
        for j in range(n_points):
            ...
            z_train[i+j] = zn[:-1][j]
        i+=n_points
    return (target_w, w_train), (target_y, y_train), (target_z, z_train)

### EXECUTE WAVENET
param0 = {'a':0.14, 'alfa':-0.01, 'beta':-0.01*2.54, 'eta':0.05, 'c':-0.775, 'd':1, 'kappa':0.01,
         'h':0.1, 'points':5000, 'n_Iapp':3500, 'n_Iapp_new':10, 'I_max':0.1, 'I_min':0.08,
         'resolution':1, 'n_sf':5, 'fscale':'bicubic', 'bool_lineal':True, 'bool_scale':True,
         'generateIapp':True, 'generateNewIapp':True, 'shuffle':False}

outputs = ['W_predict.txt', 'Y_predict.txt', 'Z_predict.txt']
pesos = ['weights_w.txt', 'weights_y.txt', 'weights_z.txt']
...
def Nagumo_3D():
    ...
    ## Gràfics
    w, y, z = all_data
    visual(param, w[0], y[0], z[0])

def visual(param, w_target, y_target, z_target):
    ...
    f2.close()
    f3.close()
    phase_portrait(w_target, y_target, z_target, w_predict, y_predict, z_predict)
    ...
    if answer == 'y':
        ...
        time_graphic(z_target, param['Iapp'], z_predict, 'z output')
...
def phase_portrait(wout, yout, zout, w_predict, y_predict, z_predict):
...
if __name__ == "__main__":    
    ...
    print('Execution time:', Timer(lambda: Nagumo_3D()).timeit(number=1), 's')
